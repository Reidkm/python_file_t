...
class Detector(object):
    ...
    def run(self, depth_img, rgb=None, plyname=None, vizname=None):
        #
        ...
        #     img_viz = Detector.mark_missing_pixels(rgb, depth_img, np.array([100, 0, 0]))
        #     # --- 显示线段聚类的结果
    ...
    def viz_result(self, rgb, results, vizname):
        # ----- 检测结果可视化
        if vizname is not None and results['state'] == 'OK' and results['mask'] is not None:
            # ---- Mark the pixels without depth value in red
            img_viz = Detector.mark_missing_pixels(rgb, depth_img, np.array([100, 0, 0]))
...
if __name__ == '__main__':
    ...
    depth_filepaths = get_depth_filepaths('/home/reid/catkin_ws/pallet_imgae_20190923/10cm')
    # -----------------------------------
    m_detector = Detector()
    ...
    for depth_filepath in depth_filepaths:
        ...
        m_detector.viz_result(rgb_img, result_details, vizname=depth_filepath[:-4]+'viz.jpg')
