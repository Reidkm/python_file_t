...
from axonopenni.utils import (inherit_properties, HandleObject, _py_to_ctype_obj, ClosedHandle, InitializationError,
                          OpenNIError)
...
_default_dll_directories.append("/home/reid/axonopenni_python_20190824/AXonOpenNI-Linux-x64-2.3/Redist")
...
def initialize(dll_directories=_default_dll_directories):
    ...
    for dlldir in dll_directories:
        ...
        try:
            os.chdir(dlldir)
            print("本次初始化加载的动态链接库路径为：%s" % fullpath)  # 打印加载的.dll路径
            c_api.load_dll(fullpath)
            res = c_api.oniInitialize(c_api.ONI_API_VERSION)  # 增加初始化返回值
...
def unload():
    ...
    _openni2_initialized = False
    c_api.oniShutdown()
    print("断开设备连接，初始化标志为：",_openni2_initialized)   # 个人定义
...
class Device(HandleObject):
    ...
    def _reopen(self):
        ...
        if self._mode:
            ...
            status = c_api.oniDeviceOpen(self._orig_uri, ctypes.byref(self._handle))
            #print("#oniDeviceOpen ：", status)
    ...
    def enumerate_uris(cls):
        ...
        count = ctypes.c_int()
        status = c_api.oniGetDeviceList(ctypes.byref(pdevs), ctypes.byref(count))
#        print("**The Return Sataus of oniGetDeviceList is : ", status)   # 个人添加
        uris = [pdevs[i].uri for i in range(count.value)]
#        print("**ALL Finded DeviceList:", uris)

        status2 = c_api.oniReleaseDeviceList(pdevs)
        uris2 = [pdevs[i].uri for i in range(count.value)]
#        print("**After Release:", uris2)
#        print("**The Return Sataus of oniReleaseDeviceList is : ", status2)  # 个人添加
    ...
    def invoke(self, command_id, data, size=None):
        ...
        result = c_api.oniDeviceInvoke(self._handle, command_id, ctypes.byref(data), size)
    ...
    def set_depth_color_sync_enabled(self, enable):
        if enable:
            result = c_api.oniDeviceEnableDepthColorSync(self._handle)
            return result
        else:
...
class VideoFrame(HandleObject):
    ...
    def _close(self):
        if is_initialized():
            ...
            #print("**oniFrameRelease is Done!")
        self._frame = ClosedHandle

    def add_ref(self):
        c_api.oniFrameAddRef(self._handle)
...
class VideoStream(HandleObject):
    ...
    def _close(self):
        if is_initialized():
            ...
            print("**oniStreamDestroy is Done!")
        self.camera = None
...
## 自定义read_pframe
    def read_pframe(self):
        frame = c_api.OniFrame
    ...
    def register_new_frame_listener(self, callback):
        ...
        status = c_api.oniStreamRegisterNewFrameCallback(self._handle, cbobj, None, ctypes.byref(cb_handle))
        return status
    ...
    def set_property(self, property_id, obj, size=None):
        ...
        if size is None:
            size = ctypes.sizeof(obj)
        result = c_api.oniStreamSetProperty(self._handle, property_id, ctypes.byref(obj), size)
        return result
...
def get_bytes_per_pixel(format):
    ...
    num = c_api.oniFormatBytesPerPixel(format)
    return num


def get_depth_value_unit(format):
    #c_api.oniGetDepthValueUnit_mm.restype = ctypes.c_float
    u = c_api.oniGetDepthValueUnit_mm(format)
    return u
...
class DeviceListener(HandleObject):
    def __init__(self):
        ...
        result = c_api.oniRegisterDeviceCallbacks(self._callbacks, None, ctypes.byref(handle))
        print("设备监听器调用结果为：", result)
        HandleObject.__init__(self, handle)
