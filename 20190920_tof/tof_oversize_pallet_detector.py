...
class Detector(object):
    def __init__(self):
        self.oversize_detection_params = {
            ...
            'PALLET_LEG_MASK': np.ones((4, 10, 2), dtype=np.uint8),  # voxel
            # 'PALLET_LEG_MASK': np.ones((8, 10, 3), dtype=np.uint8),  # voxel
            'GRID_UNIT': 10,                # mm
            'GRID_OFFSET': [500, 200, 0],   # [x, y, z] mm
            'GRID_OFFSET_FLOOR': 500,       # mm, floor = Y + 300
            'GRID_SIZE': [80, 120, 140],    # (X, Y, Z) voxel
            'GRID_UNIT_CELEBRATE': 12.2,    # mm, 实际上每个体素对应的物理尺寸
    ...
    def get_plastic_mask():
        ...
        for i_line in range(height):
            x_bar = round(1.0 * dx * i_line / height)
            z_bar = round(1.0 * dz * i_line / height)
            # print(x_bar, z_bar)
    ...
    def run(self, depth_img, plyname=None, vizname=None):
        #
        ...
        # -----------------------------------
        # floor_height = int(self.oversize_detection_params['GRID_OFFSET_FLOOR'] / unit)  # voxels
        pallet_dist, floor_height = Detector.height_map(voxel_count)
        ...
        results = Detector.find_edges_of_pallet_and_goods(depth_img,
                                                          ...
                                                          self.mask_color_dict)
        # --- Step 4
        x_pixel = 1000000  # initial invalid value
        x_mm = 1000000
        if results['state'] == 'OK':
        ...
        if vizname is not None and results['state'] == 'OK' and results['mask'] is not None:
            ...
            img_viz = Detector.mark_missing_pixels(rgb_img, depth_img, np.array([100, 0, 0]))
            # --- 显示线段聚类的结果
...
if __name__ == '__main__':
    ...
    for depth_filepath in depth_filepaths:
        ...
        dist_x_pixel, dist_x_mm = m_detector.run(depth_img, vizname=depth_filepath[:-4]+'viz.jpg')
        #
